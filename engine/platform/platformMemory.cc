...
// Nodes: 32 or 64 bytes
// TREE_NODE_UNUSED_SIZE is not defined because it rounds up to 64 bytes in x64!
#define PAGE_RECORD_UNUSED_SIZE (IS_X64? 8 : 4)
// Headers: 64 or 88 bytes
#define HEADER_UNUSED_SIZE (IS_X64? 8 : 4)
...
struct PageRecord
{
   ...
   void *basePtr;
   U32 unused[PAGE_RECORD_UNUSED_SIZE];  // even out the record to 32 or 64 bytes...
                   // so if we're on a 32-byte cache-line comp, the tree nodes
                   // will cache better
};

static_assert(sizeof(PageRecord) == sizeof(TreeNode), "PageRecord size must match TreeNode size.");
static_assert(sizeof(Header) == sizeof(FreeHeader), "Memory headers must share the same size.");
static_assert(sizeof(FreeHeader) == sizeof(AllocatedHeader), "Memory headers must share the same size.");
...
void* dRealloc(void* in_pResize, dsize_t in_size)
{
    ...
    return mi_realloc(in_pResize, in_size);
}
