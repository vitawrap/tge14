...
//                                        globals
//
constexpr U32 getNextPow2(U32 io_num)
{
	S32 oneCount = 0;
	S32 shiftCount = -1;
	while (io_num) {
...
constexpr U32 getBinLog2(U32 io_num)	// viwrap (12/2022) slightly faster binlog2
{
	// find the log base 2 of 32-bit v
	const S32 MultiplyDeBruijnBitPosition[32] =
	{
		0, 9, 1, 10, 13, 21, 2, 29, 11, 14, 16, 18, 22, 25, 3, 30,
		8, 12, 20, 28, 15, 17, 24, 7, 19, 27, 23, 6, 26, 5, 4, 31
	};

	io_num |= io_num >> 1; // first round down to one less than a power of 2 
	io_num |= io_num >> 2;
	io_num |= io_num >> 4;
	io_num |= io_num >> 8;
	io_num |= io_num >> 16;

	return MultiplyDeBruijnBitPosition[(U32)(io_num * 0x07C4ACDDU) >> 27];
}
...
// compiler is GCC 4+ and supports __builtin_offsetof
#elif defined(TORQUE_COMPILER_GCC) && (__GNUC__ > 4)
#include <stddef.h>
#define Offset(x, cls) __builtin_offsetof(cls, x)
#define OffsetNonConst(x, cls) __builtin_offsetof(cls, x)
