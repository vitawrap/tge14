...
void fxFoliageReplicator::renderObject(SceneState* state, SceneRenderImage*)
{
	...
	ColorF amb(1.f, 1.f, 1.f);
	F32 temp = amb.red;
	amb.red = amb.blue;
	amb.blue = temp;
	getLightingAmbientColor(&amb);
			...
			for (U32 idx = 0; idx < mFrustumRenderSet.mVisObjectSet.size(); idx++)
			{
				...
				if (pFoliageItem->LastFrameSerialID != mFrameSerialID)
				{
					...
					if (Distance >= MinimumViewDistance && Distance <= MaximumViewDistance)
					{
						...
						// Blend with scene ambience
						ColorF color = amb * Luminance;

						// Draw Billboard.
						glBegin(GL_QUADS);
							// Set Blend Function.
							glColor4f(color.red, color.green, color.blue, ItemAlpha);
							...
							// Set Ground Blend.
							if (mFieldData.mGroundAlpha < 1.0f) 
								glColor4f(color.red, color.green, color.blue, mFieldData.mGroundAlpha < ItemAlpha ? mFieldData.mGroundAlpha : ItemAlpha);
