...
//--------------------------------------------------------------------------
// Camera recoil with spread
//--------------------------------------------------------------------------

class CameraRecoil : public CameraFX
{
    typedef CameraFX Parent;

    VectorF mRecoilOffset;
    VectorF mRandomSpread;

    // Recoil function:
    F32 interpolate(F32 x);

public:
    CameraRecoil();

    virtual void init();
    void setRecoilOffset(VectorF& pos) { mRecoilOffset = pos; }
    void setRandomSpread(VectorF& spr) { mRandomSpread = spr; }
