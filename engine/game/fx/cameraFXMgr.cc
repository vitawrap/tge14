...
{
    mRecoilOffset.x += gRandGen.randF(-mRandomSpread.x, mRandomSpread.x);
    mRecoilOffset.y += gRandGen.randF(-mRandomSpread.y, mRandomSpread.y);
    mRecoilOffset.z += gRandGen.randF(-mRandomSpread.z, mRandomSpread.z);
}

void CameraRecoil::update(F32 dt)
{
    Parent::update(dt);
