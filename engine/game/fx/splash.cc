...
void Splash::renderObject(SceneState* state, SceneRenderImage*)
{
   ...
   if (mCausingObject)
   {
       Point3F wpos = mCausingObject->getRenderPosition();
       MatrixF mxObj = getRenderTransform();
       wpos.z = getPosition().z;
       mxObj.setColumn(3, wpos);
       setRenderTransform(mxObj);
   }
...
SplashRing Splash::createRing()
{
   ...
   for( U32 i=0; i<numPoints; i++ )
   {
      ...
      point.position = getRenderPosition() + startOffset;
