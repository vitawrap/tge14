...
PlayerData::PlayerData()
{
   ...
   // 13/07/22: READDED JET FIELDS FROM TGE 1100
   mRechargeRate = 0.256;
   jetForce = 26.2 * 90;
   jetEnergyDrain = 0.8;
   minJetEnergy = 1;
...
void PlayerData::initPersistFields()
{
   ...
   addField("canSwim", TypeBool, Offset(canSwim, PlayerData));
   ...
   addField("rechargeRate", TypeF32, Offset(mRechargeRate, PlayerData));
   ...
   addField("crouchBoundingBox", TypePoint3F, Offset(boxCrouchSize, PlayerData));
   ...
   addField("jumpSound",     TypeAudioProfilePtr, Offset(sound[JumpSound], PlayerData));
...
void PlayerData::packData(BitStream* stream)
{
   ...
   stream->write(maxJetForwardSpeed);
   ...
   stream->write(boxCrouchSize.x);
   stream->write(boxCrouchSize.y);
   stream->write(boxCrouchSize.z);
...
void PlayerData::unpackData(BitStream* stream)
{
   ...
   stream->read(&maxJetForwardSpeed);
   ...
   stream->read(&boxCrouchSize.x);
   stream->read(&boxCrouchSize.y);
   stream->read(&boxCrouchSize.z);
...
bool Player::onNewDataBlock(GameBaseData* dptr)
{
   ...
   S32 headUpSeq = shape->findSequence("headUp");
   if (headUpSeq != -1)
   {
       mHeadUpThread = mShapeInstance->addThread();
       mShapeInstance->setBlendEnabled(mHeadUpThread, true);
       mShapeInstance->setSequence(mHeadUpThread, headUpSeq, 0.f);
       mShapeInstance->setTimeScale(mHeadUpThread, 0.f);
   }
   else
       mHeadUpThread = 0;
...
void Player::updateMove(const Move* move)
{
   ...
   // Set swim state for later
   mSwimming = mWaterCoverage > 0.45f;
   ...
   else if (mSwimming && mDataBlock->canSwim)   // NEW: Basic swimming movement!
   {
       // Remove acc into contact surface (should only be gravity)
       // Clear out floating point acc errors, this will allow
       // the player to "rest" on the ground.
   ...
   // Jet behavior
   mJetting = mState == MoveState && mDamageState == Enabled && move->trigger[4] && mEnergy >= mDataBlock->minJetEnergy;
   ...
   if (mJetting) {
       ...
       if (jvl > 0) {
           jv *= 1 / jvl;
           F32 dot = mDot(mVelocity, jv);
           F32 hacc = 1;
           if (dot > 0)
               if (dot > mDataBlock->maxJetForwardSpeed)
                   hacc = 0;
               else
                   hacc = 1 - (dot / mDataBlock->maxJetForwardSpeed);
           if (hacc > mDataBlock->maxJetHorizontalPercentage)
               hacc = mDataBlock->maxJetHorizontalPercentage;
           jv *= hacc * mDataBlock->jetForce;
           jv += zv * (1 - hacc) * mDataBlock->jetForce;
       }
       else
           jv.set(0, 0, mDataBlock->jetForce);
       ...
       if (mDrag != 0.0f && mWaterCoverage > 0.25f) {
           ...
           Point3F vertDragVel(0, 0, getMax(mVelocity.z, 0.0f));
           jv += vertDragVel * mDrag * TickSec * 0.8;
       }

       // Update velocity
       acc += jv * (TickSec / mDataBlock->mass);
   }
   ...
   if (move->trigger[2] && !isMounted() && canJump())
   {
      ...
      if (zSpeedScale <= mDataBlock->maxJumpSpeed)
      {
         ...
         if (isGhost() && mDataBlock->sound[PlayerData::JumpSound])
             alxPlay(mDataBlock->sound[PlayerData::JumpSound], &getTransform());
      }
   }
   else
      if (jumpSurface) {
   ...
   if (!isGhost()) {
      ...
      if(!inLiquid && mWaterCoverage >= 1.0f) {
         ...
         inLiquid = true;
          ...
          // Play exitwater sound
          if (getVelocity().len() >= mDataBlock->exitSplashSoundVel && !isMounted())
            alxPlay(mDataBlock->sound[PlayerData::ExitWater], &getTransform());
          
          // Leave pretty ripples
          Point3F point;
          mWorldBox.getCenter(&point);
          point.z = mWorldBox.min.z + (mWaterCoverage * mWorldBox.len_z());
          createSplash(point);
...
{
   // Become a corpse when we're disabled or destroyed (dead).
   if (mDamageState == Enabled) {
...
void Player::updateLookAnimation()
{
   ...
   // Something to block from walls here
}
...
void Player::setHeadUp(bool value)
{
    mHeadUp = value;
    if (isServerObject())
        setMaskBits(ActionMask);
...
U64 Player::packUpdate(NetConnection *con, U64 mask, BitStream *stream)
{
...
ConsoleMethod(Player, isCrouched, bool, 2, 2, "Is the player crouching?")
{
    return object->getIsCrouching();
}
   ...
   if (pointInWater( curPos )) {
      if (!pointInWater( mLastPos )) {
         ...
         if (gClientContainer.castRay(mLastPos, curPos,
            ...
            // Moved out of createSplash to separate the logic
