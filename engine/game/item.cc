...
U64 Item::packUpdate(NetConnection *connection, U64 mask, BitStream *stream)
{
   U64 retMask = Parent::packUpdate(connection,mask,stream);
...
void Item::renderImage(SceneState* state, SceneRenderImage* image)
{
   ...
   F32 finalScale = 1.f;
   if (mDataBlock->maxCamScale > 1.f)
   {
       F32 pDx = (getPosition() - state->getCameraPosition()).lenSquared(); // Render dist
       F32 fov = GameGetCameraFov();
       RectI vp;
       dglGetViewport(&vp);
       F32 d = .5f * mSqrt(vp.extent.x * vp.extent.x + vp.extent.y * vp.extent.y);
       F32 pD1 = d / mTan(fov * .5f);   // 1:1 dist
       finalScale = mClampF(pDx / pD1, 1.f, mDataBlock->maxCamScale);
   }
   ...
   mat.scale(Point3F(finalScale, finalScale, finalScale));
   Parent::setRenderTransform(mat);
