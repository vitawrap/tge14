...
static bool initLibraries()
{
   ...
   // Original size: 3 << 20 (3 meg)
   FrameAllocator::init(1 << 24);      // 16mb frame allocator buffer
...
static F32 fpsScriptReal;
static F32 fpsScriptVirtual;
...
void fpsUpdate()
{
   ...
   if (update > 0.5f)
   {
       fpsScriptVirtual = 1.0f / fpsVirtual;
       fpsScriptReal = 1.0f / fpsReal;
...
// ---------------------------------------------------------
// FPS commands, to make up for the old script variables...
// ---------------------------------------------------------

ConsoleFunction(getVirtualFPS, S32, 1, 1, "get virtual FPS (used to be `$fps::virtual`)")
{
    return fpsScriptVirtual;
}

ConsoleFunction(getRealFPS, S32, 1, 1, "get real FPS (used to be `$fps::real`)")
{
    return fpsScriptReal;
}

ConsoleFunction(getRealtimeFPS, S32, 1, 1, "get real-time, unthrottled FPS.")
{
    return 1.f / fpsReal;
}
