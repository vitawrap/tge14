...
//--------------------------------------------------------------------------
F32 Projectile::getUpdatePriority(CameraScopeQuery *camInfo, U64 updateMask, S32 updateSkips)
{
...
void Projectile::updateSound()
{
   ...
   else if(!mHidden)
   {
      if (mSoundHandle == NULL_AUDIOHANDLE)
      {
         Point3F pos = getPosition();   // GCC doesn't like taking the address of an rvalue
         mSoundHandle = alxPlay(mDataBlock->sound, &getRenderTransform(), &pos);
      }
...
void Projectile::processTick(const Move* move)
{
   ...
   {
      if (mDataBlock->explodeOnDeath)
      {
          mCurrVelocity.normalizeSafe();
          explode(mCurrPosition, mCurrVelocity, 0);
      }
       else
        deleteObject();
      return;
      ...
      if(stream->writeFlag(len > 0.02))
   {
      ...
      if (bool(mSourceObject))
      {
         ...
         if (stream->writeFlag(ghostIndex != -1))
         {
            ...
            //stream->writeRangedU32(U32(mSourceObjectSlot), 0, ShapeBase::MaxMountedImages - 1);
            stream->writeSignedInt(mSourceObjectSlot, getBinLog2(ShapeBase::MaxMountedImages));
         }
...
void Projectile::unpackUpdate(NetConnection* con, BitStream* stream)
{
   ...
   if (stream->readFlag())
   {
      ...
      if (stream->readFlag())
      {
         ...
         mSourceObjectSlot = stream->readSignedInt(getBinLog2(ShapeBase::MaxMountedImages));
