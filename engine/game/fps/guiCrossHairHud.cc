...
class GuiCrossHairHud : public GuiBitmapCtrl
{
   ...
   S32      mAxisLineSize;
   S32      mTrackedImageSlot;
   F32      mRadiusDelta;       ///< Used for animation

protected:
   void drawDamage(Point2I offset, F32 damage, F32 opacity);
   void drawAxes(F32 radius, Point2I center);
...
void GuiCrossHairHud::initPersistFields()
{
   ...
   addField( "axisLineSize", TypeS32, Offset(mAxisLineSize, GuiCrossHairHud) );
   addField( "trackedSlot", TypeS32, Offset(mTrackedImageSlot, GuiCrossHairHud) );
   endGroup("Damage");		
}
...
void GuiCrossHairHud::onRender(Point2I offset, const RectI &updateRect)
{
   ...
   // Render axes
   ShapeBase* pSB = conn->getControlObject();
   ShapeBase::MountedImage* pImage = NULL;
   if (pSB && (pImage = pSB->getImageStruct(mTrackedImageSlot)))
   {
       ShapeBaseImageData::StateData* state = pImage->state;
       S32 precision;
       if (state && ((precision = state->precision) >= 0))
       {
           Point2I dstPoint(
               (offset.x + (mBounds.extent.x >> 1)),
               (offset.y + (mBounds.extent.y >> 1))
           );
           drawAxes(precision, dstPoint);
       }
   }
...
/**
   Display cross-hair axes on the bitmap.
   This is a support funtion, called by onRender.
*/
void GuiCrossHairHud::drawAxes(F32 radius, Point2I center)
{
    // Animate radius adjustment in time
    U32 nowMS = Platform::getVirtualMilliseconds();
    static U32 lastTimeMS = nowMS;
    F32 dt = (nowMS - lastTimeMS) * 0.001f * 16.f;
    lastTimeMS = nowMS;
    if (mRadiusDelta != radius)
    {
        mRadiusDelta = mRadiusDelta + dt * (radius - mRadiusDelta);
        if (mFabs(mRadiusDelta - radius) < 0.1f)
            mRadiusDelta = radius;
    }

    // Define axis lines (<^>v)
    S32 line = mAxisLineSize;
    struct LineI { Point2I a, b; };
    LineI axes[4] = {
        { Point2I( center.x - mRadiusDelta - line, center.y ), Point2I( center.x - mRadiusDelta, center.y ) },
        { Point2I( center.x, center.y - mRadiusDelta - line ), Point2I( center.x, center.y - mRadiusDelta) },
        { Point2I( center.x + mRadiusDelta, center.y ),        Point2I( center.x + mRadiusDelta + line, center.y ) },
        { Point2I( center.x, center.y + mRadiusDelta),         Point2I( center.x, center.y + mRadiusDelta + line ) },
    };
    
    // Draw highlight
    Point2I offset(1, 1);
    ColorI axisColor = mProfile->mFillColorHL;
    dglDrawLine(axes[0].a - offset, axes[0].b - offset, axisColor);
    dglDrawLine(axes[1].a - offset, axes[1].b - offset, axisColor);
    dglDrawLine(axes[2].a - offset, axes[2].b - offset, axisColor);
    dglDrawLine(axes[3].a - offset, axes[3].b - offset, axisColor);

    // Draw shade
    axisColor = mProfile->mFillColor;
    dglDrawLine(axes[0].a, axes[0].b, axisColor);
    dglDrawLine(axes[1].a, axes[1].b, axisColor);
    dglDrawLine(axes[2].a, axes[2].b, axisColor);
    dglDrawLine(axes[3].a, axes[3].b, axisColor);
}
