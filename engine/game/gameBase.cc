...
F32 GameBase::getUpdatePriority(CameraScopeQuery *camInfo, U64 updateMask, S32 updateSkips)
{
   updateMask;
...
void GameBase::unpackUpdate(NetConnection *con, BitStream *stream)
{
   ...
   if (stream->readFlag()) {
      ...
      if (!Sim::findObject(id,dptr) || !setDataBlock(dptr))
         con->setLastError("Invalid datablock sent to GameBase::unpackUpdate()");
   }
}
