...
ShapeBaseImageData::ShapeBaseImageData()
{
   ...
   camRecoilOffset.set(0.0f, 10.0f, 0.0f);
   camRecoilSpread.set(1.0f, 1.0f, 0.0f);
   camRecoilDuration = 1.5f;
...
bool ShapeBaseImageData::onAdd()
{
   ...
   for (U32 i = 0; i < MaxStates; i++) {
      ...
      if (statesLoaded == false) {
         ...
         s.camRecoil = stateCamRecoil[i];
         s.precision = statePrecision[i];
      }
...
void ShapeBaseImageData::initPersistFields()
{
   ...
   addField("casingCount",       TypeS32,            Offset(casingCount,         ShapeBaseImageData));
   ...
   addField("camRecoilOffset", TypePoint3F, Offset(camRecoilOffset, ShapeBaseImageData));
   addField("camRecoilSpread", TypePoint3F, Offset(camRecoilSpread, ShapeBaseImageData));
   addField("camRecoilDuration", TypeF32, Offset(camRecoilDuration, ShapeBaseImageData));
   ...
   addField("stateCamRecoil", TypeBool, Offset(stateCamRecoil, ShapeBaseImageData), MaxStates);
   addField("statePrecision", TypeS32, Offset(statePrecision, ShapeBaseImageData), MaxStates);
...
void ShapeBaseImageData::packData(BitStream* stream)
{
   ...
   if( stream->writeFlag( casing ) )
   {
      ...
      stream->write(casingCount);
   }

   // Cam shake fields
   mathWrite(*stream, camRecoilOffset);
   ...
   for (U32 i = 0; i < MaxStates; i++)
      if (stream->writeFlag(state[i].name && state[i].name[0])) {
         ...
         stream->writeFlag(s.camRecoil);
         stream->writeSignedInt(s.precision, 7);
...
void ShapeBaseImageData::unpackData(BitStream* stream)
{
   ...
   for (U32 i = 0; i < MaxStates; i++) {
      if (stream->readFlag()) {
         ...
         s.camRecoil = stream->readFlag();
         s.precision = stream->readSignedInt(7);
...
void ShapeBase::getImageTransform(U32 imageSlot,MatrixF* mat)
{
   ...
   if (image.dataBlock) {
      ...
      if (data.useEyeOffset && isFirstPerson()) {
         ...
         getMountTransform(imageSlot, &nmat);
...
void ShapeBase::spawnCamRecoil(ShapeBaseImageData& imageData)
{
    if (GameConnection* con = getControllingClient())
        if (con->getControlObject() == this && con->isFirstPerson())
        {
            CameraRecoil* camRecoil = new CameraRecoil();
            camRecoil->setDuration(imageData.camRecoilDuration);
            camRecoil->setRecoilOffset(imageData.camRecoilOffset);
            camRecoil->setRandomSpread(imageData.camRecoilSpread);
            camRecoil->init();
            gCamFXMgr.addFX(camRecoil);
        }
}
...
void ShapeBase::setImageState(U32 imageSlot, U32 newState,bool force)
{
   ...
   // Spawn cam shake
   if (isGhost() && stateData.camRecoil)
       spawnCamRecoil(imageData);
...
void ShapeBase::registerLights(LightManager * lightManager, bool lightingScene)
{
   ...
   for (S32 i = 0; i < MaxMountedImages; i++)
   {
      ...
      if (imageData != NULL && imageData->lightType != ShapeBaseImageData::NoLight)
      {
         ...
         getRenderMountTransform(i, &posMat);
...
void ShapeBase::ejectShellCasing( U32 imageSlot )
{
   ...
   // Casing count is at least raised to 1 if a casing is specified.
   S32 debrisCount = getMax(imageData->casingCount, 1);
   for (S32 i = 0; i < debrisCount; ++i)
   {
       Point3F randomDir;
