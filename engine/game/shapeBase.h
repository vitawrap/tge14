...
struct ShapeBaseImageData: public GameBaseData {
  ...
  public:
   ...
   struct StateData {
      ...
      bool camRecoil;               ///< Set this state to spawn the image's cam shake (for recoil)
                                    ///  state.
      S32 precision;                ///< This is nothing but a cosmetic value that works with
                                    ///  GuiCrossHairHud to define the axis distance from the center.
   ...
   bool                    stateCamRecoil             [MaxStates];
   S32                     statePrecision             [MaxStates];
   ...
   /// @name Cam Recoil
   /// @{
   VectorF  camRecoilOffset;        ///< Camera shake Frequency
   VectorF  camRecoilSpread;        ///< Camera shake Amplitude
   F32      camRecoilDuration;      ///< Camera shake Duration
   /// @}

   /// @name Shell casing data
   /// @{
   DebrisData *   casing;              ///< Information about shell casings.
   S32            casingCount;         ///< (13/02/2020) Number of casings to eject
...
protected:
 ...
 protected:
   ...
   /// Update buoyancy and drag properties
   void updateContainer();

   /// Spawn shake (used for image recoil fx)
   void spawnCamRecoil(ShapeBaseImageData&);
...
public:
   ...
   F32 getUpdatePriority(CameraScopeQuery *focusObject, U64 updateMask, S32 updateSkips);
