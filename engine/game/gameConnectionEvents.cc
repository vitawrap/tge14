...
IMPLEMENT_CO_CLIENTEVENT_V1(CameraShakeEvent);
...
{
   mGuaranteeType = GuaranteedOrdered;
   mObj = NULL;
   mIndex = index;
   mTotal = total;
...
void SimDataBlockEvent::notifyDelivered(NetConnection *conn, bool )
{
   ...
   // This can easily be ripped out if we choose not to use a queue anymore
   // ConsoleMethod( GC, transmitDatablocks, ...) in gameConnection.cc should be edited accordingly if so.
...
void SimDataBlockEvent::process(NetConnection *cptr)
{
   if(mProcess)
   {
      ...
      {
         U8 buf[MaxPacketDataSize];
         BitStream stream(buf, MaxPacketDataSize);
...
CameraShakeEvent::CameraShakeEvent(
    ...
    VectorF const& amp,
    F32 duration)
    : mFalloff(falloff), mFrequency(freq), mAmplitude(amp), mDuration(duration)
{
    mGuaranteeType = Guaranteed;
}

void CameraShakeEvent::pack(NetConnection* conn, BitStream* stream)
{
    stream->write(mFalloff);
    stream->write(mDuration);
    mathWrite(*stream, mFrequency);
    mathWrite(*stream, mAmplitude);
}

void CameraShakeEvent::write(NetConnection* conn, BitStream* stream)
{
    stream->write(mFalloff);
    stream->write(mDuration);
    mathWrite(*stream, mFrequency);
...
void CameraShakeEvent::unpack(NetConnection* conn, BitStream* stream)
{
    stream->read(&mFalloff);
    stream->read(&mDuration);
    mathRead(*stream, &mFrequency);
    mathRead(*stream, &mAmplitude);
}

void CameraShakeEvent::process(NetConnection* _conn)
{
    CameraShake* shake = new CameraShake();
    shake->init();
    shake->setAmplitude(mAmplitude);
    shake->setFrequency(mFrequency);
    shake->setDuration(mDuration);
    shake->setFalloff(mFalloff);
