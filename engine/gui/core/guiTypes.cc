...
void GuiCursor::onLeftClick(const Point2I &pos)
{
    mLastClickTime = Platform::getVirtualMilliseconds();
}
...
void GuiCursor::render(const Point2I &pos)
{
   ...
   // Animate a quick grow animation, for responsivity.
   U32 now = Platform::getVirtualMilliseconds();
   static const U32 time = 100U;   // 100ms
   static const F32 invTime = 1.f / time;
   if (now < (mLastClickTime + time))
   {
       F32 ratio = F32(now - mLastClickTime) * invTime;
       texWidth = 1 + (texWidth * ratio);
       texHeight = 1 + (texHeight * ratio);
   }
   ...
   RectI rect(renderPos.x, renderPos.y, texWidth, texHeight);
