...
void GuiBitmapCtrl::initPersistFields()
{
   ...
   addField("center", TypeBool,     Offset(mCenter,     GuiBitmapCtrl));
   endGroup("Misc");		
}
		        ...
		        for(int x = 0; x < xdone; ++x)
		        {
		 	        srcRegion.set(0,0,texture->bitmapWidth,texture->bitmapHeight);
  			        dstRegion.set( ((texture->bitmapWidth*x)+offset.x)-xshift,
   		            ...
   		            dglDrawBitmapStretchSR(texture,dstRegion, srcRegion);
		        }
        }
        else if (mCenter)
        {
            Point2I dstPoint(
                (offset.x + (mBounds.extent.x >> 1)) - (texture->bitmapWidth >> 1),
                (offset.y + (mBounds.extent.y >> 1)) - (texture->bitmapHeight >> 1)
            );
            RectI dstRegion( dstPoint, Point2I(texture->bitmapWidth, texture->bitmapHeight) );
            dglDrawBitmapStretch(texture, dstRegion);
        }
        else
        {
