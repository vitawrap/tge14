...
extern U16* sgDetailPolyList;
extern U32  sgDetailPolyListSize;
...
void Interior::renderARB(const bool useAlarmLighting, MaterialList* pMaterials, const LM_HANDLE instanceHandle)
{
   ...
   glEnable(GL_TEXTURE_2D);
   glTexEnvi(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_MODULATE);
       // Setup for GL_COMBINE env mode (essentially toggleable masking state)
       //glTexEnvi(GL_TEXTURE_ENV, GL_COMBINE_ALPHA, GL_INTERPOLATE);
       //glTexEnvi(GL_TEXTURE_ENV, GL_SOURCE1_ALPHA, GL_TEXTURE2_ARB);
   ...
   //glTexEnvi(GL_TEXTURE_ENV, GL_OPERAND2_ALPHA, GL_ZERO);
   //glTexEnvi(GL_TEXTURE_ENV, GL_OPERAND2_RGB, GL_ZERO);
   ...
   for (i = 0; i < sgActivePolyListSize; i++) {
      ...
      if (baseName != currentlyBound0) {
         ...
         if (instanceHandle != -1)
         {
             glName = gInteriorLMManager.getHandle(mLMHandle, instanceHandle, baseName)->getGLName();
             AssertFatal(glName, "Interior::renderARB: invalid glName for texture handle");
         }
   ...
   if (smRenderEnvironmentMaps && sgEnvironPolyListSize) {
      ...
      for (i = 0; i < sgEnvironPolyListSize; i++) {
         ...
         if (baseName != currentlyBound1) {
            ...
            //if (mEnvironMasks[rSurface.textureIndex])
            //{
            //    baseName = mEnvironMasks[rSurface.textureIndex]->getGLName();
            //}
            //else
            //    baseName = -1;
            //if ((baseName != -1) && (baseName != currentlyBound2))
            //{
            //    glTexEnvi(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_COMBINE);
            //    glActiveTextureARB(GL_TEXTURE2_ARB);
            //    glBindTexture(GL_TEXTURE_2D, baseName);
   ...
   } // endif (environment render on);

   //NQ:Render Detail maps (revisit at some points, to put more details for more Texture units)
   if (smRenderDetailMaps && sgDetailPolyListSize){
      currentlyBound1 = (U32)-1;
      currentlyBound0 = (U32)-1;
      ...
      for (i = 0; i < sgDetailPolyListSize; i++) {
         const Surface& rSurface = mSurfaces[sgDetailPolyList[i]];
         ...
         // Setup the detail map...
         baseName = mDetailMaps[rSurface.textureIndex]->getGLName();
         if (baseName != currentlyBound1) {
            glActiveTextureARB(GL_TEXTURE1_ARB);
            glBindTexture(GL_TEXTURE_2D, baseName);
            currentlyBound1 = baseName;
         ...
         // And the colors...
         F32 baseLevel = mDetailFactors[rSurface.textureIndex];
         F32 scale = 1.f / mDetailScales[rSurface.textureIndex];
         ...
         for (U32 j = rSurface.windingStart; j < rSurface.windingStart + rSurface.windingCount; j++) {
            ...
            glColor4f(1, 1, 1, baseLevel);
            glMultiTexCoord2fARB(GL_TEXTURE0_ARB, s, t);
            glMultiTexCoord2fARB(GL_TEXTURE1_ARB, s * scale, t * scale);
            glVertex3fv(mPoints[mWindings[j]].point);
         }
         glEnd();
      }
   } // if (detail map render on);
   ...
   // Reset tex env for unit 1 for safety
   //glTexEnvi(GL_TEXTURE_ENV, GL_SOURCE1_ALPHA, GL_TEXTURE1_ARB);
                            ...
                            U32            numLightSurfaces)
{
   ...
   for (U32 i = 0; i < numLightSurfaces; i++)
   {
      ...
      {
          glActiveTextureARB(GL_TEXTURE1_ARB);
          glBindTexture(GL_TEXTURE_2D, mMaterialList->getMaterial(rSurface.textureIndex).getGLName());
      }
      ...
      F32 imax = 1.f / fmaxf(fmaxf(pInfo->mColor.red, pInfo->mColor.green), pInfo->mColor.blue);
      glColor4f(pInfo->mColor.red * imax, pInfo->mColor.green * imax, pInfo->mColor.blue * imax, factor);
      ...
      for (U32 j = rSurface.windingStart; j < rSurface.windingStart + rSurface.windingCount; j++)
      {
         ...
         if (bDoDeinit)
         {
             Interior::TexGenPlanes* pGenUV = &mTexGenEQs[rSurface.texGenIndex];
             glMultiTexCoord2fARB(GL_TEXTURE1_ARB, 
                 mDot(rPoint, pGenUV->planeX) + pGenUV->planeX.d,
                 mDot(rPoint, pGenUV->planeY) + pGenUV->planeY.d
             );
         }

         glTexCoord2f(((splane.distToPlane(rPoint) / mr) + 1.0) * .5f,
                      ((tplane.distToPlane(rPoint) / mr) + 1.0) * .5f);
         glVertex3fv(mPoints[mWindings[j]].point);
      }
      glEnd();
   }
