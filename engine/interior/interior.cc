...
//NQ: Added for detail map support
U16*            sgDetailPolyList	     = NULL;
U32             sgDetailPolyListSize  = 0;
...
Interior::Interior()
{
   ...
   VECTOR_SET_ASSOCIATION(mEnvironMasks);
   VECTOR_SET_ASSOCIATION(mEnvironFactors);
//NQ: Added for detail maps support
   VECTOR_SET_ASSOCIATION(mDetailMaps);
   VECTOR_SET_ASSOCIATION(mDetailFactors);
...
Interior::~Interior()
{
   ...
   for (i = 0; i < mEnvironMasks.size(); i++) {
       delete mEnvironMasks[i];
       mEnvironMasks[i] = NULL;
   }

   for (i = 0; i < mDetailMaps.size(); i++) {
      delete mDetailMaps[i];
      mDetailMaps[i] = NULL;
   }
...
bool Interior::prepForRendering(const char* path)
{
   ...
   U32 size = mMaterialList->getMaterialCount();
   mEnvironMaps.setSize(size);
   mEnvironMasks.setSize(size);
   mEnvironFactors.setSize(size);
   mValidEnvironMaps = 0;
   mValidEnvironMasks = 0;
   //NQ: Added for detail maps support
   mDetailMaps.setSize(size);
   mDetailFactors.setSize(size);
   mDetailScales.setSize(size);
   mValidDetailMaps = 0;
   for (U32 i = 0; i < size; i++) {
      mEnvironFactors[i] = 1.0f; mDetailFactors[i] = 1.0f; mDetailScales[i] = 1.0f;
      ...
      if (pEntry != NULL) {
         ...
         if (pEntry->environMaskName != NULL) {
             mEnvironMasks[i] = new TextureHandle(pEntry->environMaskName, MeshTexture);
             mValidEnvironMasks += mEnvironMasks[i] != NULL ? 1 : 0;
         }
         else {
             mEnvironMasks[i] = NULL;
         }
         if (pEntry->detailMapName != NULL) {
            mDetailMaps[i]  = new TextureHandle(pEntry->detailMapName, InteriorTexture);
            mDetailFactors[i] = pEntry->detailMapFactor;
            mDetailScales[i] = pEntry->detailMapScale;
            mValidDetailMaps += mDetailMaps[i] != NULL ? 1 : 0;
         } else {
            mDetailMaps[i] = NULL;
         }
                                   ...
                                   const Point3F&     scale)
{
   ...
   sgFogPolyListSize     = 0;
   //NQ : Added for detail maps support
   sgDetailPolyList	 = (U16*)FrameAllocator::alloc(mSurfaces.size() * sizeof(16));
   sgDetailPolyListSize	 = 0;
   ...
   bool environmentActive = (dglDoesSupportARBMultitexture() &&
                             ...
                             mValidEnvironMaps != 0);
   //NQ: Added for detail maps support, see above comment
   bool detailActive = (dglDoesSupportARBMultitexture() && smRenderDetailMaps &&
                        mValidDetailMaps != 0);

   // Totally bare
   if (sgFogActive && useFogCoord())
   {
      if (environmentActive || detailActive) {
         // Environ, fc fog
         for (i = 0; i < outputCount; i++) {
            ...
            else if (mDetailMaps[rSurface.textureIndex] != NULL)
               sgDetailPolyList[sgDetailPolyListSize++] = output[i];
