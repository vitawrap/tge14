...
// The asm based blender looks like shit!
				...
				switch( numTexturesToBlend ) // Blend 1 square of the numTexturesToBlend bit-maps into the blend buffer
   			{
				...
				for(U32 yLumelInSquare(0); yLumelInSquare < LUMELS_PER_SQUARE_EDGE; ++yLumelInSquare)
				{
					...
					for(U32 xLumelInSquare(0); xLumelInSquare < LUMELS_PER_SQUARE_EDGE; ++xLumelInSquare)
					{
						...
						vector unsigned int col_const = (vector unsigned int) (0x1f << 11, 0x1f << 11, 0x1f << 11, 0x1f << 11);
						
						col[0] = vec_and(lumels, col_const);
						col[1] = vec_and(vec_sl(lumels, (vector unsigned int) (5, 5, 5, 5)), col_const);
						col[2] = vec_and(vec_sl(lumels, (vector unsigned int) (10, 10, 10, 10)), col_const);
						
						// One for each color component
						vector unsigned int left_component_delta;
						vector unsigned int right_component_delta;
						vector unsigned int vscan_left_component;
						vector unsigned int vscan_right_component;
						vector unsigned int vec_targetTexelsPerLumel_log2;
						vector unsigned int vec_two = (vector unsigned int) (2, 2, 2, 2);
						
						loader = (unsigned int *) &vec_targetTexelsPerLumel_log2;
						loader[0] = loader[1] = loader[2] = loader[3] = targetTexelsPerLumel_log2;
						
						transpose(4, col);	// transpose the matrix since we were using rows and now we need columns
						
						left_component_delta = vec_sr(vec_sub(col[2], col[0]), vec_targetTexelsPerLumel_log2);
						right_component_delta = vec_sr(vec_sub(col[3], col[1]), vec_targetTexelsPerLumel_log2);
						
						vscan_left_component = col[0];
						vscan_right_component = col[1];
						...
						{
							vector unsigned int across_component_delta;
							vector unsigned int hscan_component;
							
							across_component_delta = vec_sr(vec_sub(vscan_right_component, vscan_left_component), vec_targetTexelsPerLumel_log2);
							hscan_component = vscan_left_component;
							...
							{
								unsigned int *sloader;
								vector unsigned int dstcol;
								vector unsigned int index;
								vector unsigned int index_const = (vector unsigned int) (0x3F00, 0x3F00, 0x3F00, 0x3F00);
								vector unsigned int dstcol_const = (vector unsigned int) (0xf8, 0xf8, 0xf8, 0xf8);
								...
								index = vec_and(vec_sr(hscan_component, vec_two), index_const);
								address.v = vec_or(index, cur_srcbits);
								sloader = (unsigned int *) &dstcol;
								sloader[0] = alphaTable[address.s[0]];
								sloader[1] = alphaTable[address.s[1]];
								sloader[2] = alphaTable[address.s[2]];
								...
								dst_tmp.v = vec_sr(vec_sl(vec_and(dstcol, dstcol_const), (vector unsigned int) (7, 2, 0, 0)), (vector unsigned int) (0, 0, 3, 0));
								*dstbits++ = (unsigned short) (dst_tmp.s[0] | dst_tmp.s[1] | dst_tmp.s[2]);
#else
								// for some strange and nonexistent altivec processor which uses RGBA5551
								dst_tmp.v = vec_sr(vec_sl(vec_and(dstcol, dstcol_const), (vector unsigned int) (8, 3, 0, 0)), (vector unsigned int) (0, 0, 2, 0));
								*dstbits++ = (unsigned short) (dst_tmp.s[0] | dst_tmp.s[1] | dst_tmp.s[2]);
#endif
								srcbits += 4;
...
#else
                  *dst++ = texelPtr[0] | (dsize_t(texelPtr[1]) << 8) | (dsize_t(texelPtr[2]) << 16);
#endif
            }
         }
      }
