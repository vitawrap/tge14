...
//   mNewGenTextureCount = 0;
   ...
   for(S32 x = 0; x < xExt; x++)
   {
      ...
      prevCorner = TerrainRender::allocInitialPoint(Point3F((xStart + x + 1) * blockSize, yStart * blockSize, height));
...
//               glBindTexture(GL_TEXTURE_2D, hazeTexture.getGLName()); //test code to see hazetex...
               glBindTexture(GL_TEXTURE_2D, walk->handle.getGLName()); //handle to terrain material and lmap texture
               ...
               renderXFCache();    // Base terrain render pass!
               PROFILE_END();

               if(!multitextureFog) // Usually this is skipped.
               {
