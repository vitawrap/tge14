...
public:
   ...
   S64 fileOffset;            ///< Offset of data in zip file.
   S64 fileSize;              ///< Size on disk of resource block.
...
class ResDictionary
{
   ...
   S32 hashTableSize;
   DataChunker memPool;
   S64 hash(StringTableEntry path, StringTableEntry name);
   S64 hash(ResourceObject *obj) { return hash(obj->path, obj->name); }
   /// @}
...
public:
   ...
   S64 getSize(const char* filename);                 ///< Gets the size of the file
