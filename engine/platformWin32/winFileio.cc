...
#ifdef UNICODE
      char fnbuf[1024];
      convertUTF16toUTF8((const UTF16*)findData.cFileName, (UTF8 *)fnbuf, sizeof(fnbuf));
#else
...
bool Platform::getFileTimes(const char *filePath, FileTime *createTime, FileTime *modifyTime)
{
   ...
   HANDLE h = FindFirstFile((LPCWSTR)fp, &findData);
...
bool Platform::isFile(const char *pFilePath)
{
   ...
   HANDLE handle = FindFirstFile((LPCWSTR)b, &findData);
#else
