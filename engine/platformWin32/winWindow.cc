...
#ifdef UNICODE
static const UTF16 *windowClassName = (const UTF16*)L"Darkstar Window Class";
static UTF16 windowName[256] = { (UTF16)L"Darkstar Window" };
#else
...
void CreatePixelFormat( PIXELFORMATDESCRIPTOR *pPFD, S32 colorBits, S32 depthBits, S32 stencilBits, bool stereo )
{
   PIXELFORMATDESCRIPTOR src =
   {
      ...
      PFD_DOUBLEBUFFER,       // double buffered (WE DON'T NEED glFinish()!)
...
#define REG_VAL_FIREFOX "FirefoxURL"
#define REG_VAL_CHROME "ChromeHTML"
#define REG_VAL_OPERA "OperaStable"
...
bool Platform::openWebBrowser( const char* webAddress )
{
   ...
   static wchar_t sWebPathKey[512];
   static char utf8WebPathKey[512];

   {
      bool resortToExplorer = false;    // Flag of true despair
      HKEY regKey;
      DWORD size = sizeof( sWebKey );
      DWORD pathSize = sizeof(sWebPathKey);

      // Fetch browser name key
      if ( RegOpenKeyEx( HKEY_CURRENT_USER, dT("Software\\Microsoft\\Windows\\Shell\\Associations\\UrlAssociations\\http\\UserChoice"),
          0, KEY_QUERY_VALUE, &regKey ) != ERROR_SUCCESS )
      {
          Con::errorf( ConsoleLogEntry::General, "Platform::openWebBrowser - Damn Internet Explorer, really?"); 
          resortToExplorer = true;
      }

      if (!resortToExplorer)
      {
          // Fetch browser key
          if ( RegQueryValueEx( regKey, dT("ProgId"), NULL, NULL, (unsigned char *)sWebKey, &size ) != ERROR_SUCCESS ) 
          {
             Con::errorf( ConsoleLogEntry::General, "Platform::openWebBrowser - Failed to query the id key!!!" );
             return( false );
          }
          ...
          convertUTF16toUTF8((const UTF16*)sWebKey,utf8WebKey,512);

          // Store custom browser path from id
#ifdef UNICODE
          static wchar_t sBrowserPath[512];
          wsprintfW(sBrowserPath, L"SOFTWARE\\Classes\\%s\\shell\\open\\command", sWebKey);
#else
          static UTF8 sBrowserPath[512];
          dSprintf(sBrowserPath, "SOFTWARE\\Classes\\%s\\shell\\open\\command", utf8WebKey);
#endif // UNICODE

          // Fetch browser path from browser id key
          if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, sBrowserPath, 0, KEY_QUERY_VALUE, &regKey) != ERROR_SUCCESS)
          {
              Con::errorf(ConsoleLogEntry::General, "Platform::openWebBrowser - Failed to open the browser path key!!!");
              return(false);
          }

          if (RegQueryValueEx(regKey, NULL, NULL, NULL, (unsigned char*)sWebPathKey, &pathSize) != ERROR_SUCCESS)
          {
              Con::errorf(ConsoleLogEntry::General, "Platform::openWebBrowser - Failed to open the browser path!!!");
              return(false);
          }
      ...
      else
      {
          ...
          if (RegQueryValueEx(regKey, dT(""), NULL, NULL, (unsigned char*)sWebPathKey, &size) != ERROR_SUCCESS)
          {
      ...
      convertUTF16toUTF8((const UTF16*)sWebPathKey, utf8WebPathKey, 512);

#ifdef UNICODE
      char* p = dStrstr((const char*)utf8WebPathKey, "%1");
#else
      char* p = strstr((const char*)sWebPathKey, "%1");
#endif
      if (p) *p = 0;
   }
   ...
   char buf[1024];
#ifdef UNICODE
   dSprintf( buf, sizeof( buf ), "%s %s", utf8WebPathKey, webAddress );   
   UTF16 b[1024];
   convertUTF8toUTF16((UTF8 *)buf, b, sizeof(b));
#else
   dSprintf( buf, sizeof( buf ), "%s %s", sWebPathKey, webAddress );   
#endif
...
//--------------------------------------
#ifdef UNICODE
static const UTF16* TorqueRegKey = (UTF16*)dT("SOFTWARE\\GarageGames\\Torque");
#else
...
{
   HKEY regKey;
   if ( RegOpenKeyEx( HKEY_LOCAL_MACHINE, (LPCWSTR)TorqueRegKey, 0, KEY_WRITE, &regKey ) == ERROR_SUCCESS )
   {
