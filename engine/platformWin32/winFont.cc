...
#define FALLBACKFONT_PATH ".\\common\\ui\\fallback.otf"
...
GOldFont *createFont(const char *name, dsize_t size, U32 charset /* = TGE_ANSI_CHARSET */)
{
   ...
   HFONT hNewFont = CreateFont(size,0,0,0,0,0,0,0,DEFAULT_CHARSET,OUT_TT_PRECIS,0, NONANTIALIASED_QUALITY,0, (LPCWSTR)n);
#else
   HFONT hNewFont = CreateFont(size,0,0,0,0,0,0,0,charsetMap[charset],0,0, NONANTIALIASED_QUALITY,0,name);
#endif
   if(!hNewFont)
      return NULL;
...
#endif
         SIZE size;
         GetTextExtentPoint32(fontHDC, (LPCWSTR) & b, 1, &size);
         if(size.cx)
...
{
    //AddFontResource(TEXT(FALLBACKFONT_PATH));
}
...
bool WinFont::create(const char *name, U32 size, U32 charset /* = TGE_ANSI_CHARSET */)
{
    ...
    UTF16 n[512];
    convertUTF8toUTF16((UTF8 *)name, n, sizeof(n));
    mFont = CreateFont(size,0,0,0,0,0,0,0,DEFAULT_CHARSET,OUT_TT_PRECIS,0,ANTIALIASED_QUALITY,0,(LPCWSTR)n);
#else
    mFont = CreateFont(size,0,0,0,0,0,0,0,charsetMap[charset],OUT_TT_PRECIS,0,ANTIALIASED_QUALITY,0,name);
#endif
    if(mFont == NULL)
      return false;
