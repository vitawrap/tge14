...
inline Point3F::Point3F(const char* str)
    : x(0), y(0), z(0)
{
    // A faster alternative to the adopted dSscanf(argv[x], "%g %g %g", &...);
    while (*str == ' ') ++ str;
    x = dAtof(str);

    const char* spc1 = dStrchr(str, ' ');
    if (!spc1) return;
    while (*spc1 == ' ') ++ spc1;
    y = dAtof(spc1);

    const char* spc2 = dStrchr(spc1, ' ');
    if (!spc2) return;
    while (*spc2 == ' ') ++ spc2;
    z = dAtof(spc2);
}
