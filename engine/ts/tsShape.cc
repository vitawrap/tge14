...
void TSShape::assembleShape()
{
   ...
   S32 * ptr32 = alloc.copyToShape32(numNodes*5);   // Node is a struct of 5 S32s
   nodes.set(ptr32,numNodes);

   alloc.checkGuard();

   ptr32 = alloc.copyToShape32(numObjects*6,true);   // Object is a struct of 6 S32s
   if (!ptr32)
   ...
   typedef dsize_t tsMeshPtr;   // This helps keep clean code while porting for bigger address sizes.

   // read in the meshes (sans skins)...
   if (smReadVersion>15)
   {
      // straight forward read one at a time. this S32 buffer stores pointers... so we need to double it for x64.
      ptr32 = alloc.allocShape32((numMeshes + numSkins*numDetails) << (sizeof(tsMeshPtr) == 8)); // leave room for skins on old shapes
      ...
      for (i=0; i<numMeshes; i++)
      {
         ...
         if (ptr32)
            ((tsMeshPtr*)ptr32)[i] = skip ? 0 : (tsMeshPtr)mesh;
   ...
   else
   {
      ...
      ptr32 = alloc.allocShape32((meshIndexListSize + numSkins*numDetails) << (sizeof(tsMeshPtr) == 8));
      S32 next=0;
      S32 curObject = 0, curDecal = 0; // for tracking skipped meshes
