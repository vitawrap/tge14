...
ConsoleMethod(SimObject,dump, void, 2, 2, "obj.dump()")
{
   ...
   // Then dump engine methods and scr�pt methods
   ...
   if(ns)
      ns->getEntryList(&vecEngine, &vecScript);

   Con::printf("Engine Methods:");
   for(Vector<Namespace::Entry *>::iterator j = vecEngine.begin(); j != vecEngine.end(); j++)
      Con::printf("  %s() - %s", (*j)->mFunctionName, (*j)->mUsage ? (*j)->mUsage : "");

   Con::printf("Script Methods:");
   for (Vector<Namespace::Entry*>::iterator j = vecScript.begin(); j != vecScript.end(); j++)
       Con::printf("  %s()", (*j)->mFunctionName);
}
                ...
                " such as occurs on a mission change, but it's also good post-mission cleanup.")
{
   ...
   {
      SimObject *obj = (*grp)[i];
      SimDataBlock* db = dynamic_cast<SimDataBlock*>(obj);
      if (!db) continue;    // Skip non-datablock objects...

#ifdef TORQUE_DEBUG
      if (db->getRefCount())
          Con::warnf("Datablock %s (%d) has still %d users!", db->getName(), db->getId(), db->getRefCount());
#endif // TORQUE_DEBUG
   ...
   if (refs)    // If true, in good ol torque fashion we should've crashed :)
       Con::warnf("Datablocks still total to %d living references!", refs);
}
