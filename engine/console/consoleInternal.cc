...
ConsoleFunction(backtrace, void, 1, 1, "Print the call stack.")
{
   ...
   char const* noScope = "(null)";
   U32 noScopeLen = dStrlen(noScope);

   for(U32 i = 0; i < gEvalState.stack.size(); i++)
   {
      totalSize += ((name = gEvalState.stack[i].scopeName)? dStrlen(name) : noScopeLen) + 3;
...
void Namespace::getEntryList(Vector<Entry*>* engine, Vector<Entry*>* script)
{
    if (mHashSequence != mCacheSequence)
        buildHashTable();
    ...
    for (U32 i = 0; i < mHashSize; i++)
        if (mHashTable[i])
            (mHashTable[i]->mType == Entry::ScriptFunctionType? script : engine)
            ->push_back(mHashTable[i]);

    dQsort(engine->address(), engine->size(), sizeof(Namespace::Entry*), compareEntries);
    dQsort(script->address(), script->size(), sizeof(Namespace::Entry*), compareEntries);
}
