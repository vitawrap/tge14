...
typedef U64 SimTime;
typedef dsize_t SimObjectId; // Formerly U32, had to be bumped up for x64
...
protected:
  ...
  public:
   ...
   /// that are getting deleted are still used.
   /// These should be balanced and called from one side of the game only (client or server)
   /// otherwise values at the end will be false and negative.
   void incRefCount() { ++refCount; }
   void decRefCount() { --refCount; }
   S32 getRefCount() const { return refCount; }
};
