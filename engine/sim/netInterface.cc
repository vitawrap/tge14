...
static inline U32 rotlFixed(U32 x, S32 y)
{
   return (x << y) | (x >> (32 - y));
}

#define MD5STEP(f, w, x, y, z, data, s) w = x + rotlFixed(w + f(x, y, z) + data, s)

// Does not fill an initialization vector
static inline void computeMD5Chunk(U32 in[16], U32 digest[4]) {
    U32 a, b, c, d;
    a = digest[0];
    b = digest[1];
    c = digest[2];
...
ConsoleFunction(md5, char const*, 2, 2, "md5(anything) - Compute 128bit MD5 digest of input")
{
    ...
    *(U64*)&message[inLen - 8] = ((U64)realLen << 3);
    message[realLen] = 0x80;

    for (S32 i = 0; i < inLen; i += 64)
        computeMD5Chunk((U32*)(message+i), digest);
