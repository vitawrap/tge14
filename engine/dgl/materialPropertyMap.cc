...
bool MaterialPropertyMap::addMapping(const S32 argc, const char** argv)
{
   ...
   pEntry->environMaskName= NULL;
   pEntry->matType        = Default;
   pEntry->matFlags       = 0;
   pEntry->detailMapFactor= .5f;
   ...
   for (U32 i = 1; S32(i) < argc; i++) {
      ...
      if (dStrnicmp(param, "detail:", dStrlen("detail:")) == 0) {
         ...
         // Get the name
         char buffer[256];
         dStrncpy(buffer, start, 255);
         buffer[(end - start) >= 256 ? 255 : (end-start)] = '\0';
         pEntry->detailMapName   = StringTable->insert(buffer);

         // We keep default settings unless there is more
         if (*pColon)
         {
             // Go up to detail factor
             while (*pColon == ' ' || *pColon == '\t')
                 pColon++;
             pEntry->detailMapFactor = dAtof(pColon);
         ...
         if (*pColon)
         {
             // Go over detail factor onto detail scale
             while (*pColon == ' ' || *pColon == '\t')
                 pColon++;
             pEntry->detailMapScale = dAtof(pColon);
         }
      }
         ...
         // Data after envmap name is either a float factor, or a mask filename
         if ((*pColon >= '0') && (*pColon <= '9'))
             pEntry->environMapFactor = dAtof(pColon);
         else
         {
             buffer[255] = '\0'; // reuse buffer
             dStrncpy(buffer, pColon, 255);
             pEntry->environMaskName = StringTable->insert(buffer);
             pEntry->environMapFactor = 1.f;    // The BW mask texture takes care of that
         }
      }
